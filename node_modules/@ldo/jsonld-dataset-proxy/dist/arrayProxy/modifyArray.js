...
function checkArrayModification(target, objectsToAdd, proxyContext) {
    if (target[2]) {
        for (const objectToAdd of objectsToAdd) {
            ...
            const testDataset = new subscribable_dataset_1.ProxyTransactionalDataset(proxyContext.dataset, (0, dataset_1.createDatasetFactory)());
...
function modifyArray(config, proxyContext) {
    ...
    const addedNodes = added
        ? added
            .map((addedValue) => {
            return (0, getNodeFromRaw_1.getNodeFromRawValue)(key, addedValue, proxyContext);
        })
            .filter((val) => val != undefined)
        : [];
